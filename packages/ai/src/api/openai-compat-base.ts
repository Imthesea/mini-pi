...
export type ExtendedChatParams = OpenAI.Chat.Completions.ChatCompletionCreateParams & {
  reasoning_effort?: "low" | "medium" | "high";
  thinking?: { type: "enabled" };
};

/** 流式 chunk 的 delta 扩展：DeepSeek 的 reasoning_content 字段。 */
export type StreamDelta = NonNullable<OpenAI.Chat.Completions.ChatCompletionChunk.Choice["delta"]> & {
...
export function convertTools(tools: Context["tools"]): ChatCompletionTool[] {
  if (!tools) return [];
  return tools.map((t) => ({
    type: "function" as const,
    function: {
